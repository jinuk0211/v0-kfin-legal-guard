...
import type { LoanReport, LoanFinding } from "@/lib/loan-schemas"
import {
  LOAN_TAXONOMY,
  LOAN_SEVERITY_COLORS,
  LOAN_RISK_LEVEL_COLORS,
} from "@/lib/loan-schemas"
...
function SeverityBadge({ severity }: { severity?: string }) {
  ...
  const colors = LOAN_SEVERITY_COLORS[severity] ?? { bg: "bg-muted", text: "text-foreground" }
  return (
    <span
      className={cn(
...
function FindingCard({ finding, index }: { finding: LoanFinding; index: number }) {
  const [open, setOpen] = useState(false)
  const tax = LOAN_TAXONOMY[finding.taxonomy]
...
export function LoanReportView({ report, onReset }: LoanReportProps) {
          ...
          <div className="mb-2 flex flex-wrap items-center gap-3">
            ...
            {report.overall_risk_level && (
              <span
                className={cn(
                  "text-sm font-bold",
                  LOAN_RISK_LEVEL_COLORS[report.overall_risk_level]
                )}
              >
